    ...
    class Catalogue{
        ...
        function product($parameter){
            ...
            foreach($raw_data as $listOfRawProducts){
                $collectionOfListProducts[] = $this->createProductColumn($parameter['columns'], 
                    explode(',', $listOfRawProducts));
            }

            return [
                'food' => $collectionOfListProducts,
    ...
    class PopulationGenerator{
        ...
        public function createData($parameter){
            ...
            for($h=0; $h<$parameter['population_size']; $h++){
                ...
                echo '<th>Food</th><th>Protein</th><th>Calories</th><th>Fat</th><th>Fiber</th><th>Kromosom</th>';
                for($i=0; $i<$gen_length; $i++){
                    echo '<tr><td>' . $itemList['food'][$i]['food'] . '</td><td>' . 
                          $itemList['food'][$i]['protein'] . '</td><td>' .  
                          $itemList['food'][$i]['calories'] . '</td><td>' .  
                          $itemList['food'][$i]['fat'] . '</td><td>' .  
                          $itemList['food'][$i]['fiber'] . '</td><td>' .  
                          $gen[$h][$i] . '</tr>';
                }
                echo '</table>';
            }
